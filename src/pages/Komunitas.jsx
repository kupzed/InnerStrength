...
<div className='flex-col items-center flex text-center gap-2 '>
<img 
               src={komunitas1} 
...
<img 
               ...
               alt='' className='px-2 pt-6 flex-direction'/>
                ...
                <div className='pt-8 pb-20 grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 px-24'>
                    {[
                        { img: salingjaga, title: 'Saling Jaga', text: 'Saling Mendukung, Saling Merawat, Saling Jaga' },
                        { img: bugarmental, title: 'Bugar Mental', text: 'Bersama Berlari, Bersama Berkarya, Bugar Mental, Jiwa Kuat' },
                        { img: mentalsehat, title: 'Mentari Sehat', text: 'Cahaya Peduli, Menyinari Untuk Hati yang Sehat' },
                        { img: pikiransejahtera, title: 'Pikiran Sejahtera', text: 'Pikiran Bersama, Sejahtera Bersama' },
                        { img: hatiwaras, title: 'Hati Waras', text: 'Hati Satu, Kesehatan Menjalin Solidaritas' },
                        { img: bicarabaik, title: 'Bicara Baik', text: 'Bicara Baik, Hidup Baik, Jiwa Terang' }
                    ].map((komunitas, index) => (
                        <div key={index} className='flex flex-col items-center text-center p-4 border border-gray-200 rounded shadow-md mx-2 px-4'>
                            <img src={komunitas.img} alt={komunitas.title} className='w-full h-auto mb-4'/>
                            <p className='text-gray-500 text-sm'>+100 Anggota</p>
                            <h1 className='font-bold text-black text-lg mt-2'>{komunitas.title}</h1>
                            <p className='text-gray-700 text-sm mt-2 mb-4'>{komunitas.text}</p>
                            <Link to="/Login" className="px-4 py-2 bg-[#1572A1] hover:bg-blue-500 text-white text-sm outline-none rounded">Lihat Detail</Link>
                        </div>
                    ))}
                </div>
            </div>
